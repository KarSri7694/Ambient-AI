...
import { Gauge, Save, ScanSearch, ShieldCheck, ThumbsDown, ThumbsUp } from "lucide-react";
...
export function InboxPage({ privacy, resources }: { privacy: any; resources: any }) {
  ...
  const inbox = useQuery({ queryKey: ["proactive-inbox"], queryFn: async () => {
    const [items, policies] = await Promise.all([getJson<any>("/api/autonomy/inbox?limit=100"), getJson<any>("/api/autonomy/policies")]);
    return { items: items.items || [], policies: policies.policies || [] };
  ...
  return <><PageHeader eyebrow="Judgment-driven assistance" title="Proactive Inbox" description="Review timely opportunities and tune the runtime’s privacy and resource guardrails." actions={<Badge tone={items.length ? "warn" : "neutral"}>{items.length} opportunities</Badge>} />
