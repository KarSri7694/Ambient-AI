      ...
      <div className="two-column">
        <section className="space-y-3">
          ...
          {items.map((item: any) => {
            ...
            return <article className="record-card" key={item.interaction_id}><div className="record-top"><div className="flex gap-2"><Badge tone={report.status === "completed" ? "good" : "neutral"}>{report.status || "report"}</Badge><Badge>{item.source}</Badge>{report.artifact_action && <Badge tone={report.artifact_action === "merged" ? "good" : "neutral"}>{report.artifact_action}</Badge>}</div><time>{formatDate(item.created_at)}</time></div><h2>{report.title || "Untitled report"}</h2><p>{report.summary || "No summary provided."}</p><div className="meta-grid"><div><strong>Model</strong><span>{item.model || "unknown"}</span></div><div><strong>Tools</strong><span>{Array.isArray(report.tools_used) && report.tools_used.length ? report.tools_used.join(", ") : "none"}</span></div><div><strong>Artifact</strong><span>{report.artifact_path || "none"}</span></div>{report.artifact_reason && <div><strong>Organizer</strong><span>{report.artifact_reason}</span></div>}{Array.isArray(report.dedupe_notes) && report.dedupe_notes.length > 0 && <div><strong>Deduped</strong><span>{report.dedupe_notes.join("; ")}</span></div>}</div></article>;
          })}
        </section>
